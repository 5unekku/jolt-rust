...
use crate::{
    IntoJolt, IntoRolt, JPC_ShapeSubType, JPC_ShapeType, JPC_SubShapeID, Quat, Ref, RefConst, Vec3,
};
...
pub fn rotated_translated_shape(
    inner: &RefConst<JPC_Shape>,
    position: Vec3,
    rotation: Quat,
) -> Result<RefConst<JPC_Shape>, String> {
...
pub fn offset_center_of_mass_shape(
    inner: &RefConst<JPC_Shape>,
    offset: Vec3,
) -> Result<RefConst<JPC_Shape>, String> {
    ...
    create_shape_inner(|s, e| unsafe {
        JPC_OffsetCenterOfMassShapeSettings_Create(&settings, s, e)
    })
}
...
pub fn mutable_compound_shape(
    sub_shapes: &[SubShape],
) -> Result<crate::Ref<JPC_MutableCompoundShape>, String> {
    let raw: Vec<JPC_SubShapeSettings> = sub_shapes
        .iter()
        .map(|s| JPC_SubShapeSettings {
            Shape: s.shape.get(),
...
impl MutableCompoundShape {
    ...
    pub fn add_shape(
        ...
        unsafe {
            JPC_MutableCompoundShape_AddShape(
                *self.0,
                position.into_jolt(),
                rotation.into_jolt(),
                shape.get(),
    ...
    pub fn modify_shape(&mut self, index: u32, position: Vec3, rotation: Quat) {
        unsafe {
            JPC_MutableCompoundShape_ModifyShape(
                *self.0,
                index,
                position.into_jolt(),
                rotation.into_jolt(),
    ...
    pub fn modify_shape2(
        ...
        unsafe {
            JPC_MutableCompoundShape_ModifyShape2(
                *self.0,
                index,
                position.into_jolt(),
                rotation.into_jolt(),
